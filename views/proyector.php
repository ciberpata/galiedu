...
$db = (new Database())->getConnection(); // Creamos la conexión
$pin = $_GET['pin'] ?? '';
?>
<!DOCTYPE html>
<html lang="es">
...
<head>
    ...
    <style>
        ...
        .ranking-container {
            ...
            position: relative;
            height: 500px;
            /* Altura fija para que el scroll no salte */
            overflow: visible;
        }
...
<body>
    ...
    <?php 
        ...
        // IMPORTANTE: El archivo debe llamarse 'proyector.php' dentro de la carpeta del juego
        $pathProyector = dirname(__DIR__) . "/games/{$slugPartida}/proyector.php";
        
        if (file_exists($pathProyector)) {
            include $pathProyector;
        } else {
            // Alerta visual si el archivo no existe
            echo "<script>console.error('No se encontró el módulo proyector en: $pathProyector');</script>";
        }
    ?>
    ...
    <script>
        ...
        async function startGame() {
            try {
                const res = await fetch('../api/partidas.php?action=iniciar_juego', {
                    method: 'POST',
                ...
                if(json.success) {
                    ...
                    // El syncLoop se encargará de cambiar la pantalla al detectar el cambio de estado
                } else {
                    alert("Error al iniciar: " + json.error);
                }
        ...
        async function kickPlayer(sessionId) {
            if(!confirm('¿Expulsar a este jugador?')) return;
            try {
                const res = await fetch('../api/partidas.php?action=eliminar_jugador', {
                    method: 'POST',
                    body: JSON.stringify({ id_sesion: sessionId, id_partida: gameId })
                });
                const json = await res.json();
                if(json.success) {
                    // Forzamos actualización del fichero JSON para que el alumno detecte que ha sido expulsado
                    loadLobbyPlayers(gameId);
                }
            } catch(e) { console.error(e); }
        }
